#!/usr/bin/env python3
"""
Account Performance Analysis - $25k Account with 4% Risk

Calculate real account performance including:
...
def backtest_15min_swings(df_1min, df_15min, signals, target_pct=0.75):
...
def calculate_account_performance(trades_df, starting_capital=25000, risk_per_trade_pct=4.0):
    """
    Calculate account performance with position sizing.
    
    Args:
        starting_capital: Starting account size ($25,000)
        risk_per_trade_pct: Risk per trade as percentage (4% = $1,000 on $25k)
    """
    if len(trades_df) == 0:
        return None
    ...
    for _, trade in trades_df.iterrows():
        # Position size: risk $1,000 per trade initially
        # Assume we're risking the full swing range (no stops, but define max loss via options)
        # Simplified: position size = shares × P&L per share
        # For options: we'd buy contracts where premium = $1,000 max loss
        
        # Simple model: scale P&L by risk amount
        # If trade makes $1 per share and we risk $1,000, we make $1,000
        position_pnl = trade['pnl_per_share'] * (risk_dollars / abs(trade['pnl_per_share']) if trade['pnl_per_share'] != 0 else 1)
        
        # More realistic: assume 100 shares per $1,000 risk (typical for QQQ ~$400)
        # Risk per share ≈ $10, so 100 shares = $1,000 risk
        shares = 100
    ...
    return {
        ...
        'winners': len(trades_df[trades_df['pnl_per_share'] > 0]),
        'win_rate': (len(trades_df[trades_df['pnl_per_share'] > 0]) / len(trades_df)) * 100,
        'equity_curve': equity_curve,
        'trade_results': pd.DataFrame(trade_results)
    }


# Load data and run backtests
print("\n" + "="*70)
print("ACCOUNT PERFORMANCE ANALYSIS - $25K WITH 4% RISK")
print("="*70)
...
strategies = [
    (0.75, "Conservative: 75% of 15-Min Swing"),
    (1.00, "Aggressive: 100% of 15-Min Swing"),
]

for target_pct, strategy_name in strategies:
    print(f"\n{'='*70}")
    ...
    for filename, month_name in months:
        ...
        trades = backtest_15min_swings(df_1min, df_15min, signals, target_pct=target_pct)
    ...
    if all_trades:
        ...
        perf = calculate_account_performance(combined_trades, starting_capital=25000, risk_per_trade_pct=4.0)
        ...
        print(f"   Winners: {perf['winners']} ({perf['win_rate']:.1f}%)")
        print(f"   Risk Per Trade: 4% of account ($1,000 initially)")
        
        # Show first 10 trades
        print(f"\n📋 FIRST 10 TRADES:")
        print(perf['trade_results'].head(10).to_string(index=False))
