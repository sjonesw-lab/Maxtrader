"""
22-Month Backtest: Production vs Relaxed Parameters
QQQ-Only, $25K account, 5% risk per trade
Jan 2024 - Oct 2025
"""
import sys
sys.path.insert(0, '.')
...
def run_backtest(displacement: float, name: str):
    ...
    # Load data
    csv_files = sorted(glob.glob('data/polygon_downloads/QQQ_2024_*.csv'))
    csv_files += sorted(glob.glob('data/polygon_downloads/QQQ_2025_*.csv'))
...
def main():
    print("\n" + "="*80)
    print("22-MONTH BACKTEST: PRODUCTION vs RELAXED")
    print("="*80)
    ...
    if prod and relax:
        ...
        if relax['win_rate'] >= 80.5 and relax['max_dd'] <= 3.0:
            print("✅ RELAXED PARAMETERS VALIDATED")
            print(f"   Win Rate: {relax['win_rate']:.1f}% (meets 80.5% threshold)")
            print(f"   Max DD: {relax['max_dd']:.2f}% (meets 3.0% threshold)")
            print("   Safe to deploy")
        elif relax['win_rate'] > prod['win_rate'] and relax['max_dd'] <= prod['max_dd'] * 1.5:
            print("⚠️  RELAXED PARAMETERS ACCEPTABLE")
            print(f"   Better win rate: {relax['win_rate']:.1f}% vs {prod['win_rate']:.1f}%")
            print(f"   Acceptable drawdown: {relax['max_dd']:.2f}%")
        else:
            print("❌ KEEP PRODUCTION PARAMETERS")
            print(f"   Production: {prod['win_rate']:.1f}% win rate, {prod['max_dd']:.2f}% DD")
            print(f"   Relaxed: {relax['win_rate']:.1f}% win rate, {relax['max_dd']:.2f}% DD")
