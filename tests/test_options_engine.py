...
def test_build_long_option():
    ...
    position = build_long_option('long', 400.0, strikes, entry_time)
...
def test_60min_option_pricing_realistic():
    ...
    # Build long call (expiry calculated internally as 0DTE = 4:15 PM same day)
    position = build_long_option('long', spot, strikes, entry_time)
    
    # Verify cost is in realistic contract-level range ($100-$700 for 0DTE ATM)
    # Per-share premium ~$3-6 × 100 shares + commissions/slippage
    # Higher for longer time to expiry (10:23 AM = ~6 hours to 4:15 PM)
    assert position.entry_cost >= 100, f"0DTE contract cost too low: ${position.entry_cost:.2f}"
    assert position.entry_cost < 700, f"0DTE contract cost too high: ${position.entry_cost:.2f}"
    
    # Most importantly: verify this prevents unrealistic 67R outliers
    # Real contract costs make R-multiples realistic
    
    # Verify time to expiry is reasonable for 0DTE
    expiry = position.options[0].expiry
    time_delta = expiry - entry_time
    days_to_expiry = time_delta.total_seconds() / 86400.0
    assert 0.04 <= days_to_expiry <= 0.5, f"Time calculation wrong: {days_to_expiry:.4f} days"
